...
function App() {
  ...
  React.useEffect(() => {
    const subscription = DeviceEventEmitter.addListener(
      "locationSpeed",
      (locations) => {
        setLocationData(`${(locations[0].coords.speed * 3.6).toFixed(2)} km/h`);
        setLat(locations[0].coords.latitude);
        setLong(locations[0].coords.longitude);
      }
    );
...
TaskManager.defineTask(LOCATION_TRACKING, async ({ data, error }) => {
  ...
  if (data) {
    ...
    DeviceEventEmitter.emit("locationSpeed", locations);
    l1 = lat;
    l2 = long;
